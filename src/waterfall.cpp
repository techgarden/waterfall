...
static uint16_t mywwwport = 1337; // listen port for tcp/www (max range 1-254)
...
void loop(){
  ...
  if(plen!=0){
    ...
    Serial.println(buf[TCP_DST_PORT_H_P]);
    Serial.println(buf[TCP_DST_PORT_L_P]);
    // tcp port www start, compare only the lower byte
    if (buf[IP_PROTO_P]==IP_PROTO_TCP_V&&buf[TCP_DST_PORT_H_P]==0&&buf[TCP_DST_PORT_L_P]==81){
